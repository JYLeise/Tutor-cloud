    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity ) throws Exception {
        httpSecurity.formLogin( AbstractHttpConfigurer::disable)  //取消默认登录页面的使用  因为前后端分离的项目
                ...
                .authorizeHttpRequests( request ->
                        request
                                .requestMatchers(HttpMethod.POST,"/tutorsecurity/login","/tutorsecurity/register" ,"/tutorsecurity/logout").permitAll()
                                .requestMatchers(HttpMethod.GET,"/tutorsecurity/captcha").permitAll() //开放四个接口，一个注册，一个登录，一个验证码，
